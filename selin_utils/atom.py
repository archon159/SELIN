...
class Atom:
        ...
        **kwargs
        ...
        if c_type == 'dummy':
            ...
            if second_target == None: 
                ...
                self.display_str = ''
                self.display_str += f'{round(target * numerical_max[context], 1)}'
                self.display_str += f' <= {context} < '
                self.display_str += f'{round(second_target * numerical_max[context], 1)}'
    ...
    def check(
        ...
        elif self.c_type == 'numerical':
            if self.second_target == None: 
                if self.bigger:
                    ret = (x_[:, self.feature_id] >= self.target)
                else:
                    ret = (x_[:, self.feature_id] < self.target)
            else:
                ret = (x_[:, self.feature_id] < self.second_target) & (x_[:, self.feature_id] >= self.target)
    ...
    def check_atom_consequent(
        ...
        if c_type == 'dummy':
            ...
            if second_target == None:
                ...
                ids = np.where((self.train_x[:, feature_id] < second_target) & (self.train_x[:, feature_id] >= target))[0]
        else:
            raise ValueError(f'Context type {c_type} is not supported.')
